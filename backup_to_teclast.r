...
if(system2(command="sudo", args="cryptsetup status luks1-teclast", stdout=NULL) != 0){
  message("decrypting the disk\n")
  system2(command="sudo", args="cryptsetup open UUID=8e67e0e2-653e-4d66-a5eb-abb31881a45c --type luks1 luks1-teclast")
}
if(system2(command="findmnt", args="/mnt/teclast", stdout=NULL) != 0){
  message("mounting the disk\n")
  system2(command="sudo", args="mount /dev/mapper/luks1-teclast /mnt/teclast")
}
destination_prefix <- "/mnt/teclast/backup_zm/"
#
source_prefix <- "/mnt/"
...
stop("Test the following 2 lines of codes, and then copy them to backup_to_seagate.r.")
